...
export async function load(event) {
	...
	if (experimentData) {
		for (const experiment of experimentData) {
			...
			if (forceVariant && experimentId === 350095693703) {
				try {
					const response = await fetch(`http://localhost:3030/prod/mExperiments/view?url=https://miracle-rebuild.vercel.app${route}&status=in-progress`);
	...
	const pageCrumbs = {
		...
		pageName: `mb-sheets-ksp-${indices.find(indice => indice.experimentId === 350095693703)?.variantId}`,
		pageType: 'lander',
		productType: 'sheets'
	};
